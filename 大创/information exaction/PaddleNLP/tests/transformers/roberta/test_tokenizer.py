...
class RobertaBPETokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def test_full_tokenizer(self):
        tokenizer = self.tokenizer_class(self.vocab_file, self.merges_file,
    ...
    def test_space_encoding(self):
        ...
        tokenizer.add_special_tokens({
            "mask_token":
            AddedToken(mask, lstrip=True, rstrip=False)
    ...
    def test_embeded_special_tokens(self):
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                tokens_p = tokenizer_p.encode(sentence,
                                              add_special_tokens=True,
                ...
                self.assertEqual(
                    sum(tokens_p["attention_mask"]) /
                    len(tokens_p["attention_mask"]), 1)

                tokens_p_str = tokenizer_p.convert_ids_to_tokens(
                    tokens_p["input_ids"])
                ...
                self.assertSequenceEqual(tokens_p_str, [
                    "<s>", "A", ",", "<mask>", "ĠAllen", "N", "LP", "Ġsentence",
                    ".", "</s>"
                ])
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(
                ...
                tokenizer = self.tokenizer_class.from_pretrained(
                    pretrained_name, add_prefix_space=True)
                encoding = tokenizer(text,
                                     return_offsets_mapping=True,
                                     add_special_tokens=False,
                                     return_token_type_ids=None)
                self.assertEqual(encoding.offset_mapping[0],
                                 (0, len(text_of_1_token)))
                self.assertEqual(
                ...
                tokenizer = self.tokenizer_class.from_pretrained(
                    pretrained_name, add_prefix_space=False)
                encoding = tokenizer(text,
                                     return_offsets_mapping=True,
                                     add_special_tokens=False,
                                     return_token_type_ids=None)
...
class RobertaChineseTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        vocab_tokens = [
            "[UNK]", "[CLS]", "[SEP]", "[PAD]", "[MASK]", "want", "##want",
            "##ed", "wa", "un", "runn", "##ing", ",", "low", "lowest", "你", "我",
            "人", "天"
        ]

        self.vocab_file = os.path.join(
            self.tmpdirname,
            RobertaChineseTokenizer.resource_files_names["vocab_file"])
