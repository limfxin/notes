...
def parse_args(MODEL_CLASSES):
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        "sharding stage1/2/3. Stage 1: The optimizer states are partitioned across the processes, so that each process updates only its partition. Stage 2: The reduced gradients for updating the model weights are also partitioned such that each process retains only the gradients corresponding to its portion of the optimizer states. Stage 3: The model parameters are partitioned across the processes. stage3 will automatically collect and partition them during the forward and backward passes."
    )
